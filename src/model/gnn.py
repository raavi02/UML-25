...
from torch.geometric.nn import GCNConv, global_mean_pool, GATConv
...
def build_gnn_model(params: Dict[str, Any], n_keypoints: int) -> pl.LightningModule:
    ...
    skeleton_indices=convert_labels_to_indices(params["keypoints"][-1], params["skeleton"][-1])
    return ResidualGNN(
        input_dim=input_dim,
...
def build_gnn_datamodule(params: Dict[str, Any], splits: Dict[str, Any],keypoints: List[str]) -> pl.LightningDataModule:
    use_conf = params.get("use_confidence", True)
